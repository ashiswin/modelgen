<?php
	class RedemptionsConnector {
		...
		function __construct($mysqli) {
			...
			$this->createStatement = $mysqli->prepare("INSERT INTO " . RedemptionsConnector::$TABLE_NAME . "(`" . RedemptionsConnector::$COLUMN_STUDENTID . "`,`" . RedemptionsConnector::$COLUMN_EVENTID . "`) VALUES(?,?,?)");
			$this->selectStatement = $mysqli->prepare("SELECT * FROM " . RedemptionsConnector::$TABLE_NAME . " WHERE `" . RedemptionsConnector::$COLUMN_ID . "` = ?");
			$this->selectAllStatement = $mysqli->prepare("SELECT * FROM " . RedemptionsConnector::$TABLE_NAME);
			$this->deleteStatement = $mysqli->prepare("DELETE FROM " . RedemptionsConnector::$TABLE_NAME . " WHERE `" . RedemptionsConnector::$COLUMN_ID . "` = ?");
		}

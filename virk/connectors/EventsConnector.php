<?php
	class EventsConnector {
		...
		function __construct($mysqli) {
			...
			$this->createStatement = $mysqli->prepare("INSERT INTO " . EventsConnector::$TABLE_NAME . "(`" . EventsConnector::$COLUMN_NAME . "`,`" . EventsConnector::$COLUMN_DATE . "`,`" . EventsConnector::$COLUMN_START . "`,`" . EventsConnector::$COLUMN_END . "`,`" . EventsConnector::$COLUMN_MIN . "`,`" . EventsConnector::$COLUMN_MAX . "`,`" . EventsConnector::$COLUMN_REWARD . "`,`" . EventsConnector::$COLUMN_LOGOUTPASS . "`,`" . EventsConnector::$COLUMN_ADMINID . "`,`" . EventsConnector::$COLUMN_CREATED . "`) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
			$this->selectStatement = $mysqli->prepare("SELECT * FROM " . EventsConnector::$TABLE_NAME . " WHERE `" . EventsConnector::$COLUMN_ID . "` = ?");
			$this->selectAllStatement = $mysqli->prepare("SELECT * FROM " . EventsConnector::$TABLE_NAME);
			$this->deleteStatement = $mysqli->prepare("DELETE FROM " . EventsConnector::$TABLE_NAME . " WHERE `" . EventsConnector::$COLUMN_ID . "` = ?");
		}
